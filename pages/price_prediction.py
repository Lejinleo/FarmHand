...
def show_page():
    ...
    markets = ["Delhi", "Mumbai", "Chennai", "Kolkata"]
    ...
    if selected_crop in crops and selected_market in markets:
        ...
        # --- Mock Current Price ---
        np.random.seed(42)
        current_price = np.random.randint(2000, 4000)  # random current price in ₹/quintal

        # --- Mock Historical Prices ---
        days = pd.date_range(end=pd.Timestamp.today(), periods=30)
        historical_prices = np.random.randint(current_price - 500, current_price + 500, size=30)
        ...
        ax.plot(days, historical_prices, label="Historical Prices", color="blue")
        ax.plot(future_days, predicted_prices, label="Predicted Prices", color="orange", linestyle="--", marker="o")
        ax.axhline(y=current_price, color="green", linestyle=":", label=f"Current Price ₹{current_price}")

        # Format x-axis for better readability
        ax.xaxis.set_major_formatter(mdates.DateFormatter("%b %d"))
        ax.xaxis.set_major_locator(mdates.AutoDateLocator(maxticks=8))  # limit ticks
        ...
        - **Note:** {"Price expected to rise slightly due to demand." if predicted_prices[-1] > current_price else "Stable trend expected."}
        """)
    else:
